(() => {
  ...
  modal.innerHTML = `<section class="banner-gallery-panel" role="dialog" aria-modal="true" aria-labelledby="bannerGalleryTitle">
    ...
    <div class="gallery-tools"><button class="gallery-tool preset-apply" type="button" disabled>套用</button><button class="gallery-tool preset-copy" type="button" disabled>Copy JSON</button><button class="gallery-tool upload-open" type="button">Upload Banner</button><button class="gallery-tool" type="button" disabled>AI Generate · Coming Soon</button><p class="banner-sync-status" role="status" aria-live="polite"></p>
  ...
  async function verifyBanner(serverId,bannerPreset,attempts=10){
    for(let attempt=0;attempt<attempts;attempt+=1){
      const server=await ServerBloomData.getRemoteServerById(serverId);
      if(server&&server.bannerPreset===bannerPreset&&server.customBanner==='')return server;
  ...
  modal.querySelector('.preset-apply').addEventListener('click',async event=>{
    const button=event.currentTarget;
    if(!pendingPreset||button.disabled)return;
    const applied=pendingPreset;
    button.disabled=true;
    button.textContent='正在儲存…';
    setSyncStatus('正在儲存 Banner…');
    try{
      await saveBanner(activeCard,applied);
      if(!applyPreset(applied))throw new Error('無法更新 Banner 預覽');
      pendingPreset='';
      originalPreview=null;
      modal.querySelector('.preset-copy').dataset.json=`"bannerPreset":"${applied}"`;
      setSyncStatus('Banner 已同步到所有裝置','success');
    }catch(error){
      console.error('[ServerBloom] Banner 同步失敗',error);
      restorePreview();
      setSyncStatus(`同步失敗：${error.message||'未知錯誤'}`,'error');
