(() => {
  ...
  function applyPreset(id,persist=false){if(!activeCard||!setPreview(activeCard,id))return false;activeCard.dataset.bannerPreset=id;if(persist)localStorage.setItem(storageKey(activeCard),id);activeCard.dispatchEvent(new CustomEvent('serverbloom:bannerpresetchange',{bubbles:true,detail:{bannerPreset:id}}));return true}
