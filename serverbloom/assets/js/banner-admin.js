(() => {
  ...
  const presetMeta = [
    ['cyberpunk-purple','01 Cyberpunk Purple','遊戲'],['gaming-purple','02 Gaming Purple','遊戲'],
    ['anime-pink','03 Anime Pink','動漫'],['sakura-pink','04 Sakura Pink','動漫'],
    ['steam-style','05 Steam Style','科技'],['chat-night','06 Chat Night','聊天'],
    ['chat-neon','07 Chat Neon','聊天'],['tech-blue','08 Tech Blue','科技'],
    ['music-orange','09 Music Orange','音樂'],['fantasy','10 Fantasy','生活']
  ];
  let activeCard = null;
  let activeFilter = '全部';
  let uploadUrl = '';
  ...
  modal.innerHTML = `<section class="banner-gallery-panel" role="dialog" aria-modal="true" aria-labelledby="bannerGalleryTitle">
    ...
    <div class="gallery-tools"><button class="gallery-tool upload-open" type="button">Upload Banner</button><button class="gallery-tool" type="button" disabled>AI Generate · Coming Soon</button>
  ...
  const filters = ['全部','遊戲','動漫','聊天','科技','音樂','生活'];
  ...
  grid.innerHTML = presetMeta.map(([id,name,category]) => `<button class="official-banner" type="button" data-preset="${id}" data-category="${category}"><img src="${ServerBloomBanner.resolveBannerPreset(id)}" alt="${name}" loading="lazy"><span>${name}</span></button>`).join('');
  ...
  function applyPreset(id,persist=true){if(!activeCard)return;const src=ServerBloomBanner.resolveBannerPreset(id);const image=activeCard.querySelector('.card-banner-image');if(image){image.dataset.bannerManualSource='Official Banner';image.dataset.bannerSource='Official Banner';image.src=src}label(activeCard,'Official Banner');activeCard.querySelector('.bookmark').textContent='★';if(persist)localStorage.setItem(storageKey(activeCard),id)}
  function open(card){activeCard=card;modal.hidden=false;document.body.style.overflow='hidden';modal.querySelector('.upload-open').hidden=!(adminPreview&&cardName(card)===ownServerName);modal.querySelector('.upload-box').classList.remove('visible')}
  ...
  grid.addEventListener('click',event=>{const button=event.target.closest('[data-preset]');if(button)applyPreset(button.dataset.preset)});
  ...
  new MutationObserver(install).observe(document.querySelector('#serverGrid'),{childList:true});install();
})();
